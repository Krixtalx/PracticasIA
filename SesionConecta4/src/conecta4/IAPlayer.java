...
//				} else if (ganador == Conecta4.PLAYER2) {
...
//				}
		...
		/**
		 * Genera los hijos del nodo actual, podando las ramas que nunca serán seleccionadas
		 *
			...
			for (int i = 0; i < tableroActual[0].length && !podar; i++) {
				...
				if (encontrado) {
					...
					if (nivel % 2 == 0) {
						...
						if (this.valor > this.alfa) {
							this.alfa = this.valor;
						}
						if (alfa > beta) {
							podar = true;
						}
						...
						if (this.valor < this.beta) {
							this.beta = this.valor;
						}
						if (alfa > beta) {
							podar = true;
						}
		...
		/**
		 * Método encargado de ampliar crear los nuevos nodos necesarios para llegar al nivelMaximo.
		 *
		 * @param conecta
		 */
		...
		/**
		 * Construye el tablero en base a tableroActual y el nivel en el que nos encontramos
		 *
		...
		/**
		 * Función que comprueba si habrá una victoria al introducir una ficha en la pos x, y del tablero estTablero.
		 *
		 * @param estTablero
		 * @param x
		 * @param y
		 * @param conecta Num de fichas que debe de haber concatenadas para que haya una victoria
		 * @return Jugador ganador en el caso de que lo haya. 0 en caso contrario
		 */
...
//				nivelActual++;
	...
	/**
	 * Función usada para determinar que movimiento ha realizado el humano y poder asi ir al estado hijo correcto.
	 *
	...
	/**
	 * Función encargada de calcular el valor heurístico que tendrá el estado al que le pertenezca el tablero que se pasa como parametro
	 *
